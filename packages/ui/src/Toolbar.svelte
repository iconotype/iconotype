...
<div class="bar">
  ...
  {#if app.selection.size}
    ...
    <button class="ghost" onclick={() => app.selectNone()}>None</button>
    <button class="ghost" onclick={() => app.invertSelection()}>Invert</button>
    <button class="ghost danger" onclick={() => app.removeSelected()}>Remove</button>
  {:else}
    <button class="ghost" onclick={() => app.selectAll()} disabled={!app.matchCount}>Select all</button>
  {/if}
</div>
